...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/26 16:42:45 by skholodn          #+#    #+#             */
/*   Updated: 2017/02/26 16:43:07 by skholodn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

t_fract *burn_init(void)
{
	 t_fract *x;
		  ...
		  while (++map->f->j < WIN_LEN) // 800
		  {
				...
				while (++map->f->k < map->iter) // iterations
				{
					 if (fabs(map->f->z_im * map->f->z_im + map->f->z_re * map->f->z_re) > 4)
						  break;
					 map->f->z_im1 = 2 * fabs(map->f->z_re * map->f->z_im) + map->f->c_im;
